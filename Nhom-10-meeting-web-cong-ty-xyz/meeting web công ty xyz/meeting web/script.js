      ...
      <td>
        <div class="row-actions">
          ...
          <button
            ...
            ${meeting.status === "Đang diễn ra" ? 'disabled title="Không thể xóa cuộc họp đang diễn ra"' : ""}
          >Xóa</button>
        </div>
      </td>
    </tr>
...
function deleteMeeting(meetingId) {
  ...
  if (!meeting) {
    showAppMessage("Không tìm thấy cuộc họp cần xóa.", "error");
    renderAdminMeetings();
    return;
  }

  if (meeting.status === "Đang diễn ra") {
    showAppMessage("Không thể xóa cuộc họp đang diễn ra.", "error");
    renderAdminMeetings();
    return;
  }
